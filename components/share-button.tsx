...
export function ShareButton({ url = "https://warframefool.vercel.app" }: { url?: string }) {
    ...
    try {
      if (navigator.share) {
        await navigator.share({
          title: "Warframe Fool - Tier List Warframe",
  ...
  return (
    <Button type="button" variant="outline" size="sm" aria-label={copied ? "Link copiado" : "Compartilhar Warframe Fool"} onClick={() => void shareSite()} className="shrink-0">
